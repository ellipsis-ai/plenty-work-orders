...
return function(ellipsis, assignedUsers, maintenanceType, date, startIndex, workOrderJson) {
...
if (workOrderJson) {
  ...
  require('GetOpenWorkOrders')(ellipsis, assignedUsers, maintenanceType, date).then((workOrders) => processWorkOrders(workOrders));
}
...
${greeting}
 ...
 🎉 ${WOD.workOrderFilterDescription(assignedUsers, maintenanceType, date, ellipsis.team.timeZone, 0)} at this moment. ${getScheduleInfo()}`);
    return;
  }
...
${asset}${description}${taskSummary}`;
  ...
  const intro = `${WOD.workOrderFilterDescription(matchingMaintenanceType, matchingUsers, date, ellipsis.team.timeZone, numMatches)}. ${batchDescription(numMatches, startIndex, numShown)}`;
  const result = `
